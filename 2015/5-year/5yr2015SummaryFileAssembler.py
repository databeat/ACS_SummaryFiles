...
def assemble_zcta_file(concatenatedData, geoCodes, header, alikeSeqs, geo, sumLvl, EorM):
    ...
    if geo == 'us':
        outFileName = os.getcwd() + "/output/ACS2015_5_" + EorM + "_" + topic[0] + "/USA_" + sumLvlStr + ".csv"
    else:
...
def assemble_state_or_usa_file(concatenatedData, geoCodes, header, alikeSeqs, geo, EorM):
    ...
    if geo == 'us':
        outFileName = os.getcwd() + "/output/ACS2015_5_" + EorM + "_" + topic[0] + "/USA.csv"
    else:
        outFileName = os.getcwd() + "/output/ACS2015_5_" + EorM + "_" + topic[0] + "/" + geo.upper() + ".csv"
...
def assemble_all_states_file(alikeSeqs, EorM):
    ...
    sumLvl = 40
    outFileName = os.getcwd() + "/output/ACS2015_5_" + EorM + "_" + topic[0] + "/USA_All_States.csv"
...
def write_column_key(seqs, EorM):
    ...
    outFileName = os.getcwd() + "/output/ACS2015_5_" + EorM + "_" + topic[0] + "/" + "02_ColumnKey.csv"
    with open(outFileName, 'w+', newline='') as outFile:
        writer = csv.writer(outFile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        ...
        for seq in seqs:
            ...
            for col in range(6,numCols):
                columnMetadata = []
                columnMetadata.append(sheet.cell_value(0, col))
...
def write_table_key(seqs, EorM):
    ...
    # pairs for every estimate or margin-of-error column of its summary file.
    topic = get_topic_areas(seqs)
    outFileName = os.getcwd() + "/output/ACS2015_5_" + EorM + "_" + topic[0] + "/" + "01_TableKey.csv"
    with open(outFileName, 'w+', newline='') as outFile:
        writer = csv.writer(outFile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        ...
        for row in range(1, numRows):
            if sheet.cell_value(row, 2) in seqs:
                tableMetadata = []
                tableMetadata.append(sheet.cell_value(row, 1))
...
def write_json_columnkey(seqs, EorM):
    ...
    topic = get_topic_areas(seqs)
    outFileName = os.getcwd() + "/output/ACS2015_5_" + EorM + "_" + topic[0] + "/" + "03_ColumnKey.json"
        ...
        for seq in seqs:
            ...
            for col in range(6,numCols):
                if col == numCols-1 and i == len(seqs):
                    columnMetadata = {"columnname": sheet.cell_value(0, col),"description":sheet.cell_value(1, col)}
...
if __name__ == '__main__':
    ...
    group.add_argument("-t", "--topics", nargs='*', type=str,
                        choices=['UnweightedCount', 'AgeSex', 'Race', 'HispanicOrigin', 'Ancestry', 'ForeignBirth',
                                 ...
                                 'HealthInsurance', 'QualityMeasures', 'Imputations'],
